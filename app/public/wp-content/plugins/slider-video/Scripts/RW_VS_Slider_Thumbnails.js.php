				...
				<script >
					...
					function imageFocus<?= $Rich_Web_VSlider_ID; ?>(focus)
					{
						...
						{
							if($imageBank<?= $Rich_Web_VSlider_ID; ?>[i].attr('src') == jQuery(focus).attr('src'))
							{
					...
					function generateThumbs<?= $Rich_Web_VSlider_ID; ?>()
					{
						...
						for(var i = 0; i < imageCount<?= $Rich_Web_VSlider_ID; ?>; i++)
						{
							var $tempObj<?= $Rich_Web_VSlider_ID; ?> = jQuery('<img id="'+i+'t" class="thumb thumb<?= $Rich_Web_VSlider_ID; ?>" src="'+$imageBank<?= $Rich_Web_VSlider_ID; ?>[i].attr('src')+'" alt="'+$imageBank<?= $Rich_Web_VSlider_ID; ?>[i].attr('onclick').split("('")[1].split("')")[0]+'" onclick="Rich_Web_VSlider_TS_Close_Video<?= $Rich_Web_VSlider_ID; ?>()"/>');
							if(i == 0)
				...
				<script>
					...
					jQuery(".gallery_thumb_img<?= $Rich_Web_VSlider_ID; ?>").each(function(){
						if( jQuery(this).attr("src") != "" ) {
							array_thumb<?= $Rich_Web_VSlider_ID; ?>.push(jQuery(this).attr("src"));
						}
					})
					var y_thumb<?= $Rich_Web_VSlider_ID; ?>=0;
					for(i=0;i<array_thumb<?= $Rich_Web_VSlider_ID; ?>.length;i++){
						jQuery("<img class='gallery_thumb_img<?= $Rich_Web_VSlider_ID; ?>' />").attr('src', array_thumb<?= $Rich_Web_VSlider_ID; ?>[i]).on("load",function(){
